...
public class LogonTimeOutThread extends BaseThread{
	 ...
	 * 构造方法
	 * <br/><br/>
	 * @param timeSpace 线程休眠时间，以 毫秒为单位
	 * @param auExpireTime AU 登录超时时间，以分为单位
	 */
	...
	private void logoffLogonTypeMap(AULogonTypeManageVO logonTypeManageVO,long expireTime){
		...
		for(AUUserManageVO userManageVO : logoutList){
			try{
				...
				logger.error("登录超时，退出["+userManageVO.getUserID()+"]在["+userManageVO.getLogonType()+"]的登录["+userManageVO.getSessionID()+"]异常",e);
			}
		}
	}
