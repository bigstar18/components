...
@Service("tm_espotMonitorService")
public class EspotMonitorService
  extends StandardDao
{
  ...
  {
    String str = "select round(count(*)/5,1) as num,sysdate  from e_order o,dual where o.ordertime >= to_date('" + paramString + "','yyyy-MM-dd HH24:mi:ss')";
    ...
    executeUpdateBySql(addSql(23, "order", i, paramString, ""));
    return paramString;
  }
  
  public String addTradeNumMonitor(String paramString, int paramInt)
  {
    String str = "select round(count(*)/5,1) as num,sysdate from e_trade t,dual where t.time >= to_date('" + paramString + "','yyyy-MM-dd HH24:mi:ss')";
    List localList = queryBySql(str);
    BigDecimal localBigDecimal = (BigDecimal)((Map)localList.get(0)).get("NUM");
    int i = Double.valueOf(Math.ceil(localBigDecimal.doubleValue())).intValue();
    SimpleDateFormat localSimpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    Timestamp localTimestamp = (Timestamp)((Map)localList.get(0)).get("SYSDATE");
    paramString = localSimpleDateFormat.format(localTimestamp);
    executeUpdateBySql(addSql(23, "trade", i, paramString, ""));
    return paramString;
  }
