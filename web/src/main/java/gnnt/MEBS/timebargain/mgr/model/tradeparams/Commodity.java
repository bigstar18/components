...
{
  private static final long serialVersionUID = -4452551149016049452L;
  ...
  @ClassDiscription(name="是否含税", description="0 不含税(default)   1 含税")
  ...
  public void setTradeTime(List tradeTime)
  {
    if ((tradeTime != null) && (tradeTime.size() > 0))
    {
      ...
      while (it.hasNext())
      {
        CommodityTradeProp btp = new CommodityTradeProp();
        Commodity commodity = new Commodity();
        TradeTime tt = new TradeTime();
        commodity.setCommodityID(this.commodityID);
        btp.setCommodity(commodity);
  ...
  public String getTradeTime()
  {
    ...
    while (it.hasNext())
    {
      CommodityTradeProp btp = (CommodityTradeProp)it.next();
