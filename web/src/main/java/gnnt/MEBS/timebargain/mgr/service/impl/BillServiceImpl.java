  ...
  implements BillService
{
  ...
  public Map<String, List<?>> getBillList(String sqlName)
  {
    ...
    Map<String, List<?>> billAndCmdtySortMap = new HashMap();
    billAndCmdtySortMap.put("cmdtySort", this.tradeParamsDao
      .executeSelect(sqlName));
  ...
  public int addBill(String[] idsAndQuantity, String commodityID, String remarkHdden)
    throws Exception
  {
    ...
    for (int i = 0; i < idsAndQuantity.length; i++)
    {
      ...
      billQuantity = (billQuantity + billQuantitytemp.doubleValue());
    }
    ...
    List<Map<Object, Object>> billList = this.billDao.queryBySql(billSQL);
    this.logger.debug("*******根据仓单号查出仓单详细信息  成功！*******");
    if ((billList == null) || (billList.size() == 0)) {
      return -2;
    }
    GageBill gageBill = new GageBill();
    Map<Object, Object> billMap = (Map)billList.get(0);
    ...
    this.logger.debug("将要插入抵顶表中的数据：CommodityID【" + gageBill.getCommodityID() + "】firmId【" + gageBill.getFirmID() + "】quantity【" + gageBill.getQuantity() + "】");
    String gageBillPrimaryKey = this.billDao.addGageBill(gageBill);
    this.logger.debug("*******向仓单抵顶表插入数据  成功！*******");
    
    int validQuantity = this.billDao.existenceOfValidGageBill(firmId, 
      commodityID);
    if (validQuantity == -1)
    {
      this.billDao.executeUpdateBySql("insert into T_ValidGageBill (FirmID,CommodityID,Quantity,FrozenQty) values ('" + 
        firmId + 
        "','" + 
        commodityID + 
        "'," + 
    ...
    else
    {
      ...
      this.billDao
        .executeUpdateBySql("update T_ValidGageBill vg set vg.Quantity = " + 
        validQuantity + 
        " where vg.FirmID = '" + 
        firmId + 
    ...
    for (Map<Object, Object> map : billList)
    {
      billFrozen = new BillFrozen(gageBillPrimaryKey, String.valueOf(map
        .get("STOCKID")), Integer.valueOf(0), currDate);
  ...
  public Long billCancel(String billId, String firmId, Long billNum, String commodityId, Long operation, Long id)
    throws Exception
  {
    this.logger.debug("********************仓单撤销service********************");
    
    String validGageBillQuerySql = "select v.firmID, v.commodityID, v.quantity, v.frozenQty from T_ValidGageBill v where v.firmID = '" + firmId + "' AND v.commodityID = '" + commodityId + "' for update";
    List<Map<Object, Object>> validGageBillList = this.billDao.queryBySql(validGageBillQuerySql);
    
    Map<Object, Object> validGageBill = (Map)validGageBillList.get(0);
    this.logger.debug("可用仓单表：FIRMID【" + validGageBill.get("FIRMID") + "】COMMODITYID【" + validGageBill.get("COMMODITYID") + "】QUANTITY【" + validGageBill.get("QUANTITY") + "】FROZENQTY【" + validGageBill.get("FROZENQTY") + "】");
    
    Long usableQuantity = Long.valueOf(((BigDecimal)validGageBill.get("QUANTITY")).longValue() - ((BigDecimal)validGageBill.get("FROZENQTY")).longValue());
    ...
    String sql = "update T_ValidGageBill v set v.quantity = " + (((BigDecimal)validGageBill.get("QUANTITY")).longValue() - billNum.longValue()) + "where v.firmID = '" + firmId + "' AND v.commodityID = '" + commodityId + "'";
