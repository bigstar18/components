  ...
  extends AjaxCheckDemo
{
  public String searchForceCloseMoney()
  {
    ...
    double money = 0.0D;
    String sql = "select FN_T_ComputeForceCloseQty('" + order.getFirmId() + "','" + order.getCommodityId() + "'," + order.getBS_Flag() + "," + 1 + "," + order.getSpecPrice() + "," + order.getPrice() + ") money from dual";
    ...
    if ((money > 0.0D) && (relUsefulFunds < 0.0D)) {
      compareQuantity = Math.ceil(relUsefulFunds * -1.0D / money);
    }
