  ...
  extends StandardService
{
  ...
  public void insertSection(TradeTime tradeTime)
  {
    for (int i = 1; i < 8; i++)
    {
      String sql = "insert into T_A_DaySection (WeekDay,SectionID,Status,ModifyTime) values (" + i + ", " + tradeTime.getSectionID() + ",0,sysdate)";
      executeUpdateBySql(sql);
    }
  }
      ...
      if (weeks != null)
      {
        ...
        if ((list != null) && (list.size() > 0)) {
          for (int k = 0; k < list.size(); k++)
          {
            ...
            String sectionIDN = map.get("SECTIONID");
            
            String sql = "update T_A_DaySection set Status = 1, ModifyTime = sysdate where WeekDay = " + ri + " and SectionID = " + sectionIDN;
            executeUpdateBySql(sql);
          }
        }
      }
