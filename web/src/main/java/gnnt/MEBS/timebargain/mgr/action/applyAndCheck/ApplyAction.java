  ...
  extends EcsideAction
{
    ...
    throws Exception
  {
    ...
    if ((str2 != null) && (!"".equals(str2)))
    {
      this.request.setAttribute("gnnt_primary.content[allLike]", ">" + str2);
  ...
  public String ajaxRequest()
  {
    try
    {
      ...
      if ((str1 != null) && (!str1.equals("")))
      {
        String str3 = "";
        if ("1".equals(str2)) {
          str3 = "select s.*,t.breedName as commodityName from s_regstock s inner join t_a_breed t on s.breedid=t.breedid where 1=1 and regstockId= '" + str1 + "'";
        } else {
          str3 = "select s.*,t.breedName as commodityName,e.billfund as billFund,e.quantity as quantity from s_regstock s inner join t_a_breed t on s.breedid=t.breedid ,t_e_pledge e where s.regstockid=e.billid and '" + str1 + "' = regstockId ";
        }
        ...
        else
        {
          ...
          localStringBuilder.append("<firmId>" + ((Map)localArrayList.get(0)).get("firmID").toString() + "</firmId>");
          localStringBuilder.append("<commodityName>" + ((Map)localArrayList.get(0)).get("commodityName").toString() + "</commodityName>");
          localStringBuilder.append("<weight>" + ((Map)localArrayList.get(0)).get("weight").toString() + "</weight>");
          localStringBuilder.append("<frozenWeight>" + ((Map)localArrayList.get(0)).get("frozenWeight").toString() + "</frozenWeight>");
          if ("2".equals(str2))
          {
            localStringBuilder.append("<quantity>" + ((Map)localArrayList.get(0)).get("quantity").toString() + "</quantity>");
            localStringBuilder.append("<billFund>" + ((Map)localArrayList.get(0)).get("billFund").toString() + "</billFund>");
          }
        }
      }
      else
