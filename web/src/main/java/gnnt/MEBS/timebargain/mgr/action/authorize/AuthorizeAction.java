...
public class AuthorizeAction extends EcsideAction {
	...
	public String forceClose() throws Exception {
		...
		try {
			...
			if ((str4 != null) && (!"".equals(str4)) && (str6 != null) && (!"".equals(str6)) && (str4.equals(str6))) {
				...
				writeOperateLog(1510, "此交易商代码不属于此交易商" + str4 + "！", 0, "");
			}
		} catch (Exception localException) {
			this.logger.error("===>save err：" + localException.getMessage());
			this.request.setAttribute("prompt", localException.getMessage());
	...
	public String viewDetailForceClose() throws Exception {
		...
		StandardService localStandardService = localPageRequest.isQueryDB() ? getQueryService() : getService();
	...
	public String detailForceClose() {
		...
		try {
			...
			if ((str4 != null) && (!"".equals(str4)) && (str6 != null) && (!"".equals(str6)) && (str4.equals(str6))) {
				...
				((Order) localObject).setFirmID(str6);
				String str9 = localOrders.getCommodityId();
				((Order) localObject).setCustomerID(localOrders.getCustomerId());
				((Order) localObject).setCommodityID(str9);
				if ((str8 != null) && (!"".equals(str8))) {
				...
				((Order) localObject).setSpecTime(localOrders.getTimeFlag());
				((Order) localObject).setCloseFlag(localOrders.getCloseFlag());
				int j = this.authorizeService.insertCloseOrder(localLong, (Order) localObject);
				if (j == 0) {
					addReturnValue(1, 119907L);
				} else if (j == 65036) {
					addReturnValue(1, 151103L, new Object[] { "可平仓数量不足，请重新操作！" });
				} else {
					String str10 = this.authorizeService.resultOrder(j);
					addReturnValue(0, 151103L, new Object[] { str10 });
				}
			} else {
				addReturnValue(0, 151103L, new Object[] { "此交易商代码不属于此交易商" + str4 + "！" });
				writeOperateLog(1510, "此交易商代码不属于此交易商" + str4 + "！", 0, "");
			}
		} catch (Exception localException) {
			this.logger.error("===>save err：" + localException.getMessage());
