...
import gnnt.MEBS.common.mgr.beanforajax.BaseAjax.AjaxJSONArrayResponse;
  ...
  extends BaseAjax
{
  private boolean existFirm(String firmID)
  {
    ...
    PageRequest<String> pageRequest = new PageRequest(" and primary.firmID='" + firmID + "' and primary.status = 'N'");
    Page<StandardModel> page = getService().getPage(pageRequest, new Firm());
  ...
  private boolean existTradeprivilege(int type, String typeID, int kind, String kindID)
  {
    ...
    String sql = "select * from t_a_tradeprivilege t where t.type= " + 
      type + " and t.typeID = '" + typeID + "' and t.kind = " + kind + " and t.kindid = '" + kindID + "'";
