  ...
  extends EcsideAction
{
  ...
  public String addTransfer()
  {
    ...
    str6 = "select c.holdqty-c.frozenqty as qty from t_customerholdsum c where c.commodityid = '" + str1 + "' and c.customerid = '" + str2 + "' and c.bs_flag = " + str3;
    localList = getService().getListBySql(str6);
    if ((localList != null) && (localList.size() > 0))
    {
      i = Integer.parseInt(((Map)localList.get(0)).get("QTY").toString());
  ...
  public String viewByIdTransfer()
    throws Exception
  {
