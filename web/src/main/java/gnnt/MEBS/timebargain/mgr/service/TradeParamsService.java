...
public class TradeParamsService
{
  ...
  public List executeSelect(String sqlName)
  {
    return this.dao.executeSelect(sqlName);
  }
  ...
  {
    Iterator<StandardModel> it = list.iterator();
    Date date = new Date();
    while (it.hasNext())
    {
  ...
  {
    List<CommoditySettleProp> list = new ArrayList();
    String[] arrayOfString;
    int j;
    int i;
    if (settle != null)
    {
      j = (arrayOfString = settle).length;
      for (i = 0; i < j; i++)
      {
        String commodityID = arrayOfString[i];
  ...
  public List getSelectLabelValueByTable(String tableName, String label, String value, String tjStr)
    throws Exception
  {
