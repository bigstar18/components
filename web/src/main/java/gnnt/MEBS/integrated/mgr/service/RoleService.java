	...
	public void saveRoleRights(Role paramRole, String[] paramArrayOfString1, String[] paramArrayOfString2) {
		if ((paramArrayOfString2 != null) && (paramArrayOfString2.length > 0)) {
			...
			if ((paramRole.getUserSet() != null) && (paramRole.getUserSet().size() > 0)) {
				Iterator users = paramRole.getUserSet().iterator();
				while (((Iterator) users).hasNext()) {
					User localObject2 = (User) ((Iterator) users).next();
					PageRequest localObject3 = new PageRequest(
							" and primary.user.userId='" + ((User) localObject2).getUserId() + "' and primary.right.moduleId in(" + str1 + ") ");
					Page localObject4 = getDao().getPage((PageRequest) localObject3, new MyMenu());
			...
			while (((Iterator) localObject2).hasNext()) {
				Right localObject3 = (Right) ((Iterator) localObject2).next();
				int m = 1;
