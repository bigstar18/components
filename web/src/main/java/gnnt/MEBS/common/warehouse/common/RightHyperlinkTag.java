...
import gnnt.MEBS.common.warehouse.webframe.securitycheck.UrlCheck;
import gnnt.MEBS.common.warehouse.webframe.securitycheck.UrlCheckResult;
	...
	public int doEndTag() {
		try{
			...
			if(href != null && href.trim().length()>0 && !href.trim().equals("#")){
				urlCheckResult = urlCheck.check(href, user);
			}else if(action != null && action.trim().length()>0){
				urlCheckResult = urlCheck.check(action, user);
			}
