...
public class TradeRightInterceptor extends AbstractInterceptor {
	...
	public String intercept(ActionInvocation actioninvocation) throws Exception {
		logger.debug("enter   WarehouseIdInterceptor");
		...
		PageRequest pagerequest = new PageRequest(" order by primary.id");
		pagerequest.setPageSize(0x186a0);
		Page page = standardService.getPage(pagerequest, new Warehouse());
		String s = "";
		if (page.getResult() != null && page.getResult().size() > 0) {
			for (Iterator iterator = page.getResult().iterator(); iterator.hasNext();) {
				...
				Warehouse warehouse = (Warehouse) standardmodel;
				s = (new StringBuilder()).append(s).append("\"").append(warehouse.getWarehouseId()).append("-").append(warehouse.getWarehouseName())
						.append("\",").toString();
			}

			s = (new StringBuilder()).append("[").append(s.substring(0, s.length() - 1)).append("]").toString();
		}
		httpservletrequest.setAttribute("warehouseJson", s);
