package gnnt.MEBS.espot.mgr.action.parametermanage.interceptor;
...
public class TradeRightInterceptor extends AbstractInterceptor {
	...
	public String intercept(ActionInvocation paramActionInvocation) throws Exception {
		...
		PageRequest localPageRequest = new PageRequest(
				" and primary.status!='E' and (select count(t.firmId) from primary.firmModules as t where t.moduleID=23 and t.enabled='Y')>0 order by primary.firmId");
