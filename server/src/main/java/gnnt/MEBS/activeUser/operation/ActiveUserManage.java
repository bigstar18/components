...
public class ActiveUserManage {
	 ...
	 * 设置 AU 是否互踢
	 * <br/><br/>
	 * @param activeUserInitVO AU
	 * @param logonTimeOutThreadVO 用户登录超时退出线程参数
	 */
	...
	public AULogonResultVO logon(AULogonVO logonVO){
		...
		if(logonMode != LogonMode.MULTI_MODE){
			if(userManageList.size() > 0){
				...
				logoff(logoffVO);
				logger.info("新用户["+logonVO.getUserID()+"]以["+logonVO.getLogonType()+"]类型登录，踢出原用户 SessionID["+userManageVO.getSessionID()+"]");
			}
		}
		...
		logger.info("用户["+logonVO.getUserID()+"]在模块["+logonVO.getModuleID()+"]以["+logonVO.getLogonType()+"]类型登录成功，返回 SessionID ["+newUserManageVO.getSessionID()+"]");
		result.setResult(1);
	...
	public AUCompulsoryLogoffResultVO compulsoryLogoff(AUCompulsoryLogoffVO compulsoryLogoffVO){
		...
		if(compulsoryLogoffVO == null || compulsoryLogoffVO.getLogonIP() == null 
				|| compulsoryLogoffVO.getOperator() == null || compulsoryLogoffVO.getUserIDList() == null
				|| compulsoryLogoffVO.getUserIDList().size() <= 0){
			result.setRecode("-1401");
			result.setMessage("传入信息不完整");
			if(compulsoryLogoffVO != null){
				logger.info("管理员["+compulsoryLogoffVO.getOperator()+"]在["+compulsoryLogoffVO.getLogonIP()+"]地址强制退出用户["+compulsoryLogoffVO.getUserIDList()+"]信息不完整");
			}else{
	 ...
	 * @param logonOrLogoff 登录或退出 (1 登录，2 退出)
	 */
